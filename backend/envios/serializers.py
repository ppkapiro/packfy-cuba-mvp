...
class HistorialEstadoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'envio', 'estado', 'comentario', 'ubicacion', 
...
class EnvioSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Envio
        fields = ['id', 'numero_guia', 'estado_actual', 'estado_display', 'fecha_creacion', 
                  'fecha_estimada_entrega', 'descripcion', 'peso', 
                  'valor_declarado', 'remitente_nombre', 'remitente_direccion',
                  'remitente_telefono', 'remitente_email', 'destinatario_nombre',
                  'destinatario_direccion', 'destinatario_telefono', 
                  'destinatario_email', 'notas', 'creado_por', 'actualizado_por',
                  'ultima_actualizacion', 'historial']
        read_only_fields = ['id', 'numero_guia', 'fecha_creacion', 
                            'ultima_actualizacion', 'creado_por', 
                            'actualizado_por', 'historial']
    ...
    def validate_remitente_email(self, value):
        if value:
            try:
                ...
                raise serializers.ValidationError("El email del remitente no es válido")
        return value
    def validate_peso(self, value):
        ...
        if value > 1000:  # Establecer un límite superior razonable (1000 kg)
            raise serializers.ValidationError("El peso no puede exceder 1000 kg")
        return value
    
    def validate_valor_declarado(self, value):
        if value < 0:
            raise serializers.ValidationError("El valor declarado no puede ser negativo")
        return value
    ...
    # Estos métodos están definidos arriba, los eliminamos para evitar duplicación
    # def validate_remitente_telefono(self, value):
    #     if value:
    #         # Validar formato de teléfono cubano simple
    #         if not re.match(r'^\+?(\d[\s-]?){8,12}\d$', value):
    #             raise serializers.ValidationError("El teléfono debe tener un formato válido (+53 XXXXXXXX)")
    #     return value
    # 
    # def validate_destinatario_telefono(self, value):
    #     if value:

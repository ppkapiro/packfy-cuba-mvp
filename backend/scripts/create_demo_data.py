...
def create_demo_data():
    ...
    # Crear o actualizar usuarios y asociarlos a la empresa de demo
    created_users = []
    for role, user_data in usuarios.items():
        user, created = Usuario.objects.get_or_create(
            ...
            defaults={
                ...
                "es_administrador_empresa": user_data[
                    "es_administrador_empresa"
                ],
            },
        )

        # Asegurar consistencia de datos e idempotencia
        # - Username igual al email
        # - Flags de staff/superuser/rol acordes al diccionario
        # - Asignar empresa de demo si no tiene
        # - Refrescar contraseña para el entorno de demo
        changed = False
        ...
        if user.is_superuser != user_data.get("is_superuser", False):
            ...
            changed = True
        if (
            user.es_administrador_empresa
            != user_data["es_administrador_empresa"]
        ):
            user.es_administrador_empresa = user_data[
                "es_administrador_empresa"
            ]
            changed = True
        ...
        # Establecer/actualizar contraseña demo en cada arranque de dev
    ...
    # Usar usuarios existentes creados arriba
    try:
        admin_user = Usuario.objects.get(email="admin@packfy.cu")
    except Usuario.DoesNotExist:
        admin_user = created_users[0]
    try:
        operador_user = Usuario.objects.get(email="empresa@test.cu")
    except Usuario.DoesNotExist:
    ...
    direcciones = [
        "Calle 13 #456, Vedado, La Habana",
        "Ave 5ta #789, Miramar, La Habana",
        "Calle 42 #123, Playa, La Habana",
        "Paseo #567, Centro Habana",
        "Ave. 31 #890, Nuevo Vedado",
    ]
    telefonos = [
        "+53 555-1234",
        "+53 555-5678",
    ...
    for i in range(1, 21):
        ...
        ):  # Evitar que remitente y destinatario sean iguales
            destinatario = random.choice(nombres_destinatarios)
        ...
        if dias_atras > 25:
            ...
            estados_posibles = [
                Envio.EstadoChoices.RECIBIDO,
                Envio.EstadoChoices.EN_TRANSITO,
            ]
            ...
            estados_posibles = [
                Envio.EstadoChoices.RECIBIDO,
                Envio.EstadoChoices.CANCELADO,
                Envio.EstadoChoices.DEVUELTO,
                Envio.EstadoChoices.EN_TRANSITO,
            ]
            estado = random.choice(estados_posibles)
        ...
        envio = Envio(
            ...
            remitente_email=(
                random.choice(correos) if random.random() > 0.5 else None
            ),
            ...
            fecha_estimada_entrega=(
                fecha_entrega if random.random() > 0.2 else None
            ),
        ...
        if estado != Envio.EstadoChoices.RECIBIDO:
            ...
            if (
                estado == Envio.EstadoChoices.EN_TRANSITO
                or estado == Envio.EstadoChoices.EN_REPARTO
                or estado == Envio.EstadoChoices.ENTREGADO
                or estado == Envio.EstadoChoices.DEVUELTO
            ):
                estados_historico.append(
                    {
                        ...
                        "fecha": fecha_creacion
                        + timedelta(days=1 + random.randint(0, 2)),
                    }
                )
                ...
                estados_historico.append(
                    {
                        ...
                        "fecha": fecha_creacion
                        + timedelta(days=3 + random.randint(0, 2)),
                    }
                )
                ...
                estados_historico.append(
                    {
                        ...
                        "fecha": fecha_creacion
                        + timedelta(days=5 + random.randint(0, 3)),
                    }
                )
        ...
        print(
            f"✅ Creado: Envío #{envio.numero_guia} - Estado: {envio.get_estado_actual_display()}"
        )
    ...
    for role, user_data in usuarios.items():
        print(
            f"- {role.capitalize()}: {user_data['email']} / {user_data['password']}"
        )

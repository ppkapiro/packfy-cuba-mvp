...
const TenantSelector = () => {
  ...
  console.log('🏢 TenantSelector: Estado actual:', {
    isLoading,
    empresaActual,
    empresasDisponibles: empresasDisponibles.length,
  ...
  if (!isLoading && empresasDisponibles.length === 0) {
    return (
      <div className="tenant-selector error">
        <span className="error-icon">⚠️</span>
        <span>Sin empresas disponibles</span>
      </div>
    );
  }
  ...
  if (!empresaActual) {
    return (
      <div className="tenant-selector no-company">
        <span className="no-company-icon">🏢</span>

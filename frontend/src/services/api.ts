...
class PackfyApiClient {
  ...
  private configure(): void {
    ...
    console.log('🇨🇺 Packfy API configurándose...', { hostname, protocol, port });
  ...
  async makeRequest<T>(
    ...
    try {
      ...
      if (!response.ok) {
        result.error = responseData?.detail || responseData?.message || `HTTP ${response.status}`;
  ...
  async getEnvioByGuia(numeroGuia: string) {
    return this.makeRequest(`/envios/buscar_por_guia/?numero_guia=${numeroGuia}`);
  }
...
export const authAPI = {
  ...
  refresh: (refreshToken: string) => apiClient.makeRequest('/auth/refresh/', 'POST', { refresh: refreshToken }),
...
export const enviosAPI = {
  getAll: (page = 1, pageSize = 10, filters?: any) => apiClient.getEnvios(page, pageSize, filters),
  ...
  updateEstado: (id: number, estado: string) => apiClient.updateEnvioEstado(id, estado),
